...
class check():
    ...
    def pass_word(self,password):
        ...
        a=list(self.password)
        splchar=''',./<>?;'"[]{}\|=-+_)(*&^%$#@!~`'''
        if len(a)>16 or len(a)<5:
            st.write('Password limit should be between 5 to 16')
            ...
            if (u>=1 and l>=1 and c>=1 and d>=1):
                ...
                st.write('password error')
                st.write('Your password must contain One upper case,one lower case.one digit and one special character')
